...
* Use monotonic timers for timing: all interval measurements, delta calculations (e.g., `/proc` counter deltas), suspend-after intervals, and any elapsed-time logic MUST use a monotonic clock to avoid incorrect behavior when the system wall-clock changes. Prefer POSIX `clock_gettime(CLOCK_MONOTONIC)` or GLib's `g_get_monotonic_time()` (microseconds). When using GLib timer sources, measure elapsed time with a monotonic API for baselines and suspend/resume decisions rather than relying on wall-clock timestamps.
